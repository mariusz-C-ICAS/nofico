...
import { Shield, Layers, User, Plus, Trash2, Edit2, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <button
          onClick={() => {
            setEditingPerm({ pathType: 'O-S-P', accessLevel: 'READ', depth: -1, objectTypes: ['UNIT', 'ROLE', 'EMPLOYEE'] });
            setIsModalOpen(true);
          }}
            ...
            return (
              <div key={perm.id} className="bg-white rounded-3xl border border-slate-200 p-6 flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-md transition-shadow group">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditingPerm(perm); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-slate-50 rounded-xl transition-all">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={() => handleDelete(perm.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-slate-50 rounded-xl transition-all">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleSave} className="bg-white rounded-[40px] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl border border-slate-200">
                    {(['READ', 'WRITE', 'DELETE', 'ADMIN'] as AccessLevel[]).map(lvl => (
                      <button
                        key={lvl}
                        type="button"
                        onClick={() => setEditingPerm(prev => ({ ...prev!, accessLevel: lvl }))}
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-4 rounded-3xl text-sm font-black text-slate-500 uppercase tracking-widest hover:bg-slate-200 transition-all">
                Anuluj
              </button>
              <button type="submit" className="px-10 py-4 bg-indigo-600 text-white rounded-3xl text-sm font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-indigo-100">
                Zapisz Uprawnienie
              </button>
            </div>
          </form>
