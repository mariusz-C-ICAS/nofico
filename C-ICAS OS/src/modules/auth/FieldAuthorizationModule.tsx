...
import { Shield, Lock, Eye, EyeOff, Plus, Trash2, Edit2 } from 'lucide-react';
...
export const FieldAuthorizationModule: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPerm.id) {
        await updateDoc(doc(db, 'field_permissions', editingPerm.id), { ...editingPerm, updatedAt: serverTimestamp() });
      } else {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <button
          onClick={() => { setEditingPerm({ moduleName: 'HR_PAYROLL', accessType: 'HIDDEN', restrictionType: 'FIELD' }); setIsModalOpen(true); }}
              ...
              permissions.map(perm => (
                <tr key={perm.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${perm.accessType === 'HIDDEN' ? 'bg-rose-100 text-rose-700' : perm.accessType === 'READ_ONLY' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form onSubmit={handleSave} className="bg-white rounded-[40px] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-rose-600 p-8 flex items-center gap-4 text-white">
              ...
              <div>
                <h2 className="text-2xl font-black">{editingPerm?.id ? 'Edytuj Restrykcję' : 'Nowa Restrykcja Pola'}</h2>
                <p className="text-rose-100 text-xs font-bold uppercase tracking-widest">System Autoryzacji Granularnej</p>
              </div>
            </div>
