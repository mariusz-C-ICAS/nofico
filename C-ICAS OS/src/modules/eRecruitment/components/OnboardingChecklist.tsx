...
import {
  ...
  Target, CheckCircle2, Circle, ChevronDown, ChevronUp, Loader2,
} from 'lucide-react';
...
interface Task { id: string; label: string; owner: Owner; done: boolean; }
interface PhaseData { id: Phase; label: string; icon: React.ElementType; tasks: Task[]; }
interface Employee { id: string; name: string; position: string; startDate: string; avatar: string; phases: PhaseData[]; }
...
export default function OnboardingChecklist() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const snap = await getDocs(collection(db, `tenants/${activeTenantId}/onboarding`));
        const loaded: Employee[] = snap.docs.map(d => {
          ...
          return {
            ...
            avatar:    data.avatar    ?? (data.name ?? '??').split(' ').map((n: string) => n[0]).join('').slice(0, 2),
            phases:    initPhases(data.tasks ?? {}),
          };
        });
        setEmployees(loaded);
        const exp: Record<string, Phase | null> = {};
        loaded.forEach(e => { exp[e.id] = 'pre'; });
        setExpanded(exp);
      } finally {
        setLoading(false);
      }
  ...
  const toggleTask = async (empId: string, phaseId: Phase, taskId: string) => {
    const emp  = employees.find(e => e.id === empId);
    const task = emp?.phases.find(p => p.id === phaseId)?.tasks.find(t => t.id === taskId);
    const newDone = !(task?.done ?? false);

    await updateDoc(doc(db, `tenants/${activeTenantId}/onboarding`, empId), {
      [`tasks.${taskId}`]: newDone,
    });
    ...
    setEmployees(prev => prev.map(e => {
      ...
      return {
        ...
        phases: e.phases.map(ph => {
          ...
          return { ...ph, tasks: ph.tasks.map(t => t.id === taskId ? { ...t, done: newDone } : t) };
        }),
      };
    }));
  };
  ...
  const togglePhase = (empId: string, phaseId: Phase) => {
    setExpanded(prev => ({ ...prev, [empId]: prev[empId] === phaseId ? null : phaseId }));
  };
  ...
  if (employees.length === 0) {
    return <p className="text-sm italic text-slate-400 text-center py-12">Brak pracowników w onboardingu</p>;
  }
  ...
  return (
    <div className="space-y-10">
      {employees.map((emp, empIdx) => {
        ...
        return (
          <motion.div key={emp.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="p-8 flex flex-col md:flex-row justify-between gap-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-0.5">Start: {emp.startDate}</p>
                </div>
              </div>
              <div className="flex flex-col justify-center gap-2 min-w-[200px]">
                ...
                <div className="bg-slate-100 rounded-full h-2.5 overflow-hidden">
                  <motion.div className="h-2.5 bg-indigo-600 rounded-full" initial={{ width: 0 }}
                    animate={{ width: `${prog.pct}%` }} transition={{ duration: 0.6 }} />
                </div>
              ...
              {emp.phases.map(phase => {
                ...
                return (
                  <div key={phase.id} className="border-b border-slate-50 last:border-b-0">
                    <button onClick={() => togglePhase(emp.id, phase.id)}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{phaseDone}/{phase.tasks.length} zadan</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="w-24 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                          <div className="h-1.5 bg-indigo-600 rounded-full transition-all"
                    ...
                    {isOpen && (
                      <div className="px-8 pb-6 space-y-3">
                        {phase.tasks.map(task => (
                          <div key={task.id} onClick={() => toggleTask(emp.id, phase.id, task.id)}
