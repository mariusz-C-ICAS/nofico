...
import {
  ...
  ChevronUp, Tag, User, Calendar, Shield
} from 'lucide-react';
...
const MOCK_POLICIES: Policy[] = [
  {
    id: 'POL-001', name: 'Polityka Haseł', version: '3.2', owner: 'CISO',
    effectiveDate: '2026-01-01', reviewDate: '2026-12-31', status: 'Active',
    signed: 48, total: 52,
    history: [
      { version: '3.2', date: '2026-01-01', changes: 'Dodano wymóg MFA dla kont admin.' },
      { version: '3.1', date: '2025-07-01', changes: 'Zwiększono minimalną długość hasła do 14 znaków.' },
      { version: '3.0', date: '2025-01-01', changes: 'Pełna rewizja zgodnie z ISO 27001:2022.' },
    ],
  },
  {
    id: 'POL-002', name: 'Polityka Kontroli Dostępu', version: '2.1', owner: 'IT Security',
    effectiveDate: '2026-02-01', reviewDate: '2026-08-01', status: 'Active',
    signed: 52, total: 52,
    history: [
      { version: '2.1', date: '2026-02-01', changes: 'Dodano sekcję o uprzywilejowanym dostępie (PAM).' },
      { version: '2.0', date: '2025-08-01', changes: 'Rewizja procesu offboardingu.' },
    ],
  },
  {
    id: 'POL-003', name: 'Polityka Klasyfikacji Danych', version: '1.5', owner: 'Data Steward',
    effectiveDate: '2025-10-01', reviewDate: '2026-10-01', status: 'Active',
    signed: 40, total: 52,
    history: [
      { version: '1.5', date: '2025-10-01', changes: 'Dodano kategorię "Ściśle Tajne" dla danych biometrycznych.' },
    ],
  },
  {
    id: 'POL-004', name: 'Polityka Reagowania na Incydenty', version: '2.0', owner: 'SOC Manager',
    effectiveDate: '2026-03-01', reviewDate: '2026-09-01', status: 'Under Review',
    signed: 35, total: 52,
    history: [
      { version: '2.0', date: '2026-03-01', changes: 'Integracja z SIEM (Splunk). Nowe procedury eskalacji.' },
    ],
  },
  {
    id: 'POL-005', name: 'Polityka BYOD', version: '1.2', owner: 'IT Director',
    effectiveDate: '2025-06-01', reviewDate: '2026-06-01', status: 'Active',
    signed: 30, total: 52,
    history: [
      { version: '1.2', date: '2025-06-01', changes: 'Aktualizacja wymagań MDM dla iOS/Android.' },
    ],
  },
  {
    id: 'POL-006', name: 'Polityka Pracy Zdalnej', version: '2.3', owner: 'HR & IT',
    effectiveDate: '2026-01-15', reviewDate: '2026-07-15', status: 'Active',
    signed: 51, total: 52,
    history: [
      { version: '2.3', date: '2026-01-15', changes: 'Dodano wymogi VPN i bezpiecznego Wi-Fi.' },
    ],
  },
  {
    id: 'POL-007', name: 'Polityka Dopuszczalnego Użytkowania (AUP)', version: '4.0', owner: 'Compliance',
    effectiveDate: '2026-04-01', reviewDate: '2027-04-01', status: 'Draft',
    signed: 0, total: 52,
    history: [
      { version: '4.0 (draft)', date: '2026-04-01', changes: 'Pełna rewizja — dodano AI tools usage section.' },
    ],
  },
];
...
function PolicyCard({ policy }: { policy: Policy }) {
  ...
  const reviewOverdue = new Date(policy.reviewDate) < new Date('2026-05-14');

  return (
    <motion.div
      layout
      className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden hover:shadow-xl hover:shadow-slate-100 transition-all"
    >
      <div className="p-8">
...
export default function SecurityPolicies() {
  ...
  const [policies] = useState<Policy[]>(MOCK_POLICIES);

  const filtered = policies.filter(p => filter === 'All' || p.status === filter);

  const stats = {
    active: policies.filter(p => p.status === 'Active').length,
    fullySignedPct: Math.round((policies.filter(p => p.signed === p.total).length / policies.length) * 100),
    needsReview: policies.filter(p => new Date(p.reviewDate) < new Date('2026-05-14') && p.status === 'Active').length,
  };
