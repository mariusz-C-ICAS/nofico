...
import { Search, Zap, Trash2, ChevronRight, Info, ListTree, LayoutGrid as Grid, ShieldCheck } from 'lucide-react';
...
export default function CompetencyDictionary() {
  ...
  const getLocalized = (field: any) => {
    if (typeof field === 'object' && field !== null) return field[lang] || field.pl || field.en || '';
    return field || '';
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            {['ALL', 'SOFT', 'TECHNICAL', 'LEADERSHIP', 'DOMAIN'].map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filterCategory === cat ? 'bg-slate-900 text-white shadow-md' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50'}`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(comp => (
            <div
              ...
              className={`p-6 rounded-3xl border transition-all cursor-pointer group ${selectedComp?.id === comp.id ? 'border-indigo-600 bg-indigo-50/30' : 'bg-white border-slate-200 hover:border-indigo-300'}`}
            >
               <div className="flex justify-between items-start mb-3">
                  <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${comp.category === 'TECHNICAL' ? 'bg-blue-100 text-blue-600' : comp.category === 'SOFT' ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-100 text-indigo-600'}`}>
                    {comp.category}
                  </span>
                  {comp.isAiGenerated && <div className="text-amber-500" title="AI Support"><Zap size={12} /></div>}
               </div>
               <h3 className="text-sm font-black text-slate-800 mb-1">{getLocalized(comp.name)}</h3>
               <p className="text-xs text-slate-500 line-clamp-2 mb-4 font-medium italic">{getLocalized(comp.description)}</p>
      ...
      <div className="w-full lg:w-96 shrink-0">
        {selectedComp ? (
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-xl sticky top-8 animate-in slide-in-from-right-4">
             <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl"><ShieldCheck size={24} /></div>
                <div>
                   <h2 className="text-lg font-black text-slate-800 leading-none">{getLocalized(selectedComp.name)}</h2>
                   <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500 mt-1">{selectedComp.category}</p>
                </div>
             </div>
             <div className="mb-8 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1"><Info size={12} /> Definicja</h4>
                <p className="text-xs text-slate-700 leading-relaxed font-medium">{getLocalized(selectedComp.description)}</p>
             </div>
             <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-1"><ListTree size={12} /> Matryca Biegłości</h4>
                {selectedComp.proficiencyLevels?.map((pl: any, idx: number) => (
                  <div key={idx} className="relative pl-6 border-l-2 border-slate-100 pb-4 last:pb-0">
                    ...
                    <p className="text-[11px] text-slate-600 font-medium">{getLocalized(pl.behavior) || 'Brak zdefiniowanego wskaźnika zachowań.'}</p>
                  </div>
                ))}
             </div>
             <div className="mt-10 flex gap-3">
                ...
                <button onClick={() => deleteComp(selectedComp.id)} className="w-12 h-12 bg-rose-50 text-rose-500 rounded-xl flex items-center justify-center hover:bg-rose-100 transition-colors">
                   <Trash2 size={18} />
                </button>
             </div>
          </div>
