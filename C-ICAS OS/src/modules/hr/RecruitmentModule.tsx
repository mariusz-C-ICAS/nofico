...
export default function RecruitmentModule() {
  ...
  useEffect(() => {
    ...
    const unRec = onSnapshot(doc(db, 'hrSettings', activeTenantId + '_recruitments'), (docSnap) => {
      if (docSnap.exists()) setOpenings(docSnap.data().openPositions || []);
      else setOpenings([]);
    });
    const unCand = onSnapshot(doc(db, 'hrSettings', activeTenantId + '_candidates'), (docSnap) => {
      if (docSnap.exists()) setCandidates(docSnap.data().list || []);
      else setCandidates([]);
    });
    return () => { unRec(); unCand(); };
  }, [activeTenantId]);
  ...
  const handleHire = async (candidateName: string) => {
    try {
      ...
      alert(`Sukces: Zakończono rekrutację dla ${candidateName}. Utworzono profil pracownika.`);
    } catch (e) {
    ...
    <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-50">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            { id: 'sourcing', label: 'Sourcing & Agencje' },
            { id: 'pipeline', label: 'Lejek & Transfer' },
            { id: 'mass_import', label: 'Mass Import Struktury' },
            { id: 'fast_screen', label: 'Fast Match (UI)' },
            { id: 'iframes', label: 'Oferty (iFrame)', icon: <Monitor size={14} /> },
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors flex gap-1 items-center ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'}`}
            >
              {tab.icon} {tab.label}
            </button>
          ))}
         ...
         {activeTab === 'sourcing' && (
           <div className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-xl relative overflow-hidden">
                ...
                <p className="text-sm font-medium text-slate-300 max-w-2xl leading-relaxed mb-6">Pobieraj kandydatów bezpośrednio z API firm rekrutacyjnych (np. Adecco, Pracuj.pl, Randstad).</p>
           ...
           <div className="space-y-6 animate-in fade-in duration-300">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-emerald-50/50 rounded-2xl p-4 border border-emerald-100 h-full">
                     <h5 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-4">Gotowy do zatrudnienia</h5>
                     <div className="space-y-3">
                        {candidates.filter(c => c.status === 'Gotowy do zatrudnienia').map((c, idx) => {
                           ...
                           return (
                             <div key={idx} className="bg-white p-4 rounded-xl border-2 border-emerald-200 shadow-md relative group transition-all hover:scale-[1.02]">
                                ...
                                <button onClick={() => handleHire(c.name)} className="w-full bg-slate-900 text-white text-[10px] uppercase font-black py-2 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-800 shadow-lg shadow-slate-900/20">
         ...
         {activeTab === 'mass_import' && (
           <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
               <div className="text-center p-8 bg-white border border-slate-200 rounded-3xl shadow-sm">
                  <div className="w-20 h-20 bg-indigo-50 text-indigo-500 rounded-full flex items-center justify-center mx-auto mb-4"><Server size={32} /></div>
                  <h3 className="text-xl font-black text-slate-800 tracking-tight">Mass Import Struktur i Wakatów (Dino/Biedronka Scale)</h3>
                  <p className="text-sm text-slate-500 mt-2 max-w-2xl mx-auto">Wgraj plik Excel zawierający Jednostki Organizacyjne (O), Stanowiska (S), MPK oraz opisy wakatów.</p>
               </div>
               <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-3xl p-12 text-center hover:border-indigo-400 hover:bg-indigo-50/50 transition-colors">
                   {importStatus === 'idle' && (
                     <>
                        ...
                        <button
                          onClick={() => { setImportStatus('uploading'); setTimeout(() => setImportStatus('validating'), 1000); setTimeout(() => setImportStatus('validation_error' as any), 2500); }}
                   ...
                   {(importStatus as any) === 'validation_error' && (
                     <div className="py-6 text-left animate-in zoom-in-95 bg-white p-6 rounded-2xl shadow-xl border border-rose-200">
                        <div className="flex items-center gap-3 mb-4">
                           <div className="w-10 h-10 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center shrink-0"><X size={20} /></div>
                           <div>
                        ...
                        <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 font-mono text-xs text-slate-600 mb-6 max-h-40 overflow-y-auto">
                           <div className="text-rose-600 font-bold mb-2">&gt; Błąd Krytyczny na Linii: 43</div>
                        ...
                        <div className="flex gap-3 justify-end items-center border-t border-slate-100 pt-4">
                           <button onClick={() => setImportStatus('idle')} className="text-xs font-black uppercase text-slate-500 hover:text-slate-800 px-4 py-2">Wgraj Poprawiony Plik</button>
                           <button onClick={() => { window.dispatchEvent(new CustomEvent('addBgTask', { detail: { name: 'Ignoruj błędy i Importuj Resztę' } })); setImportStatus('idle'); }} className="text-xs font-black uppercase bg-indigo-50 text-indigo-700 px-4 py-2 border border-indigo-200 rounded-lg hover:bg-indigo-100 transition-colors">
            ...
            <div className="flex flex-col md:flex-row gap-8 h-full">
                <div className="w-full md:w-80 shrink-0">
                  <div className="bg-white border border-slate-200 p-6 rounded-3xl sticky top-8 shadow-sm">
                      <h4 className="text-xs font-black uppercase tracking-widest text-slate-700 mb-4 flex items-center gap-2"><Target size={16} className="text-blue-500" /> Wymagania Stanowiska</h4>
                      <div className="space-y-3">
                      ...
                      <AnimatePresence mode="popLayout">
                        {swipeIndex < tinderCandidates.length ? (
                          <motion.div
                            ...
                            exit={(info) => ({ x: (info as any)?.x > 0 ? 500 : -500, opacity: 0, rotate: (info as any)?.x > 0 ? 20 : -20, transition: { duration: 0.3 } })}
                          ...
                          >
                            <div>
                                <div className="flex justify-between items-start mb-6">
                                  <div>
                                      <h3 className="text-2xl font-black text-slate-800 tracking-tight">{tinderCandidates[swipeIndex].name}, {tinderCandidates[swipeIndex].age}</h3>
                                ...
                                <div className="space-y-4 mb-6">
                                  <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl">
                                      <h5 className="text-[10px] font-black uppercase text-emerald-600 tracking-widest mb-2 flex items-center gap-1"><CheckCircle2 size={12} /> Spełnia wymagania</h5>
                                      <div className="flex flex-wrap gap-2">{tinderCandidates[swipeIndex].match.map((m, i) => <span key={i} className="bg-white text-emerald-700 text-[10px] font-bold px-2.5 py-1 rounded-lg border border-emerald-100">{m}</span>)}</div>
                                  </div>
                                  <div className="bg-rose-50 border border-rose-100 p-4 rounded-2xl">
                                      <h5 className="text-[10px] font-black uppercase text-rose-600 tracking-widest mb-2 flex items-center gap-1"><X size={12} /> Braki kompetencyjne</h5>
                                      <div className="flex flex-wrap gap-2">{tinderCandidates[swipeIndex].missing.map((m, i) => <span key={i} className="bg-white text-rose-700 text-[10px] font-bold px-2.5 py-1 rounded-lg border border-rose-100">{m}</span>)}</div>
                                  </div>
                                </div>
                                <p className="text-xs text-slate-500 italic bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 font-medium">{tinderCandidates[swipeIndex].note}</p>
                            </div>
                            <div className="flex justify-center gap-6 mt-6">
                                <button onClick={() => handleSwipe('left')} className="w-16 h-16 bg-white border-2 border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:border-rose-500 hover:text-rose-500 hover:scale-110 active:scale-95 transition-all shadow-sm group"><X size={28} className="group-hover:rotate-12 transition-transform" /></button>
                                <button onClick={() => setSwipeIndex(0)} className="w-12 h-12 mt-2 bg-white border-2 border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:border-blue-500 hover:text-blue-500 hover:scale-110 active:scale-95 transition-all shadow-sm group"><Filter size={20} className="group-hover:rotate-180 transition-transform duration-500" /></button>
                                <button onClick={() => handleSwipe('right')} className="w-16 h-16 bg-white border-2 border-slate-200 rounded-full flex items-center justify-center text-slate-400 hover:border-emerald-500 hover:text-emerald-500 hover:scale-110 active:scale-95 transition-all shadow-sm group"><Heart size={28} className="group-hover:scale-125 transition-transform" /></button>
                            </div>
                          </motion.div>
                        ) : (
                          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 bg-slate-100 rounded-[3rem] border border-dashed border-slate-300">
                             <CheckCircle2 size={48} className="text-emerald-500 mb-4" />
                             <h4 className="text-lg font-black text-slate-800 uppercase">Koniec Kolejki</h4>
                             <p className="text-xs text-slate-500 mt-2">Przejrzałeś wszystkich dostępnych kandydatów.</p>
                             <button onClick={() => setSwipeIndex(0)} className="mt-6 px-6 py-2 bg-slate-900 text-white text-[10px] font-black uppercase rounded-xl">Zacznij od nowa</button>
                          </motion.div>
                        )}
                      </AnimatePresence>
                  </div>
