  ...
  const handleSave = async () => {
    ...
    try {
      if (editingConfig) {
        ...
        await addDoc(collection(db, 'careersIframeConfigs'), { ...formData, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      }
                    ...
                    <div key={conf.id} className="border border-slate-200 rounded-2xl p-5 hover:border-emerald-300 transition-colors bg-slate-50 relative group">
                       ...
                       <div className="mt-4 pt-4 border-t border-slate-200 flex gap-2 justify-between">
                          <div className="flex gap-2">
                             <button onClick={() => openEdit(conf)} className="p-2 text-slate-400 hover:text-emerald-600 bg-white border border-slate-200 rounded-lg transition-colors"><Settings size={14} /></button>
