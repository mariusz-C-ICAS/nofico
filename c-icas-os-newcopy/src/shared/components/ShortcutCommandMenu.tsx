...
export const SHORTCUTS: Shortcut[] = [
  { code: '/dash', label: 'Start', path: '/dashboard', description: 'Główny pulpit systemowy', keys: 'Alt+1' },
  { code: '/admin', label: 'Panel Dowodzenia', path: '/admin', description: 'Zarządzanie systemem', permission: 'roles.manage' },
  { code: '/crm', label: 'Sprzedaż i CRM', path: '/crm', description: 'Moduł sprzedaży i klientów', keys: 'Alt+2' },
  { code: '/hr', label: 'HR i Płace', path: '/hr', description: 'Kadry, wypłaty, czas pracy', keys: 'Alt+4' },
  { code: '/finance', label: 'Finanse', path: '/finance', description: 'Operacje i księgowość', keys: 'Alt+3' },
  { code: '/dms', label: 'Skarbiec (DMS)', path: '/dms', description: 'Obieg dokumentów', keys: 'Alt+5' },
                ...
                {recentShortcuts.slice(0, 5).map((s) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-[10px] font-black text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors whitespace-nowrap ml-2">
                       {s.code} {s.keys && <span className="opacity-60 ml-0.5">({s.keys})</span>}
                    </span>
                  </button>
                ))}
              </div>
