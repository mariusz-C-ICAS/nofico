/**
 * Data: 2026-05-12
 * Zmiany: Exporty międzynarodowe (FEC - Francja, GoBD - Niemcy).
 * Ścieżka: /src/modules/finance/reporting/GlobalCompliance.tsx
 */
import React, { useState } from 'react';
import { 
  Globe, ShieldCheck, Download, RefreshCw, 
  FileCode, CheckCircle2, AlertCircle, Bookmark,
  Server, Zap
} from 'lucide-react';
...
export default function GlobalCompliance() {
  ...
  const complianceJobs = [
    { name: 'FEC Export (Fichier des Ecritures Comptables)', region: 'FRANCE', status: 'ready', date: '2026-05-12' },
    { name: 'GoBD / DATEF Interface', region: 'GERMANY', status: 'ready', date: '2026-05-10' },
    { name: 'SAF-T (Standard Audit File for Tax)', region: 'OECD', status: 'scheduled', date: 'Next Sync: 2026-06-01' },
    { name: 'VAT OSS Re-calculation', region: 'EU-WIDE', status: 'completed', date: '2026-05-01' }
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
       <div className="bg-white border border-slate-100 rounded-[3rem] p-10 shadow-sm flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <button className="bg-slate-900 text-white px-10 py-5 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-2xl shadow-slate-100 hover:bg-indigo-600 transition-all flex items-center gap-2">
             <RefreshCw size={16} /> Odśwież Integracje
          </button>
       </div>

       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {complianceJobs.map((job, idx) => (
             <motion.div 
               key={idx}
               whileHover={{ y: -5 }}
       ...
       <div className="bg-slate-900 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl">
          <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_top_right,rgba(99,102,241,0.1),transparent)]"></div>
