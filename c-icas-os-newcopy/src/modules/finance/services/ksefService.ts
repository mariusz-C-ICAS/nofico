...
export async function fetchReceivedInvoices(
  ...
  const list: KsefInvoiceFA2[] = (data.invoiceHeaderList ?? []).map(
    (h: Record<string, unknown>) => ({
      ...
      sellerNip: String(h.subjectBy?.issuedByIdentifier?.identifier ?? h.sellerNip ?? ''),
      sellerName: String(h.subjectBy?.issuedByName?.tradeName ?? h.sellerName ?? ''),
      buyerNip: String(h.subjectTo?.issuedToIdentifier?.identifier ?? h.buyerNip ?? ''),
      buyerName: String(h.subjectTo?.issuedToName?.tradeName ?? h.buyerName ?? ''),
