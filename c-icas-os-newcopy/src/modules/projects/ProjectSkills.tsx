...
import { Sparkles, Users, AlertTriangle, CheckCircle, Search, Target, ShieldAlert, Plus, X } from 'lucide-react';
...
export default function ProjectSkills({ project }: { project: any }) {
  ...
  // Real data from project object or subcollection
  ...
  useEffect(() => {
    ...
    const unMaster = onSnapshot(collection(db, 'master_competencies'), (snap) => {
      setMasterCompetencies(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
  ...
  const handleAddSkill = async () => {
    ...
    try {
      ...
      await updateDoc(projectRef, {
        requiredCompetencies: arrayUnion({
  ...
  const handleRemoveSkill = async (skill: RequiredSkill) => {
    try {
      const projectRef = doc(db, 'projects', project.id);
      await updateDoc(projectRef, {
        requiredCompetencies: arrayRemove(skill)
      });
    } catch (err) {
      handleFirestoreError(err, OperationType.UPDATE, 'projects');
    }
  ...
  const aiSuggestions = useMemo(() => {
    ...
    return employees.map(emp => {
      ...
      requiredSkills.forEach(req => {
        // Emp skills are { name: string, level: number }
            ...
            <div className="mb-4 p-4 border border-indigo-100 bg-indigo-50/30 rounded-xl space-y-3">
              ...
              >
                ...
                {masterCompetencies.map(c => (
                  <option key={c.id} value={c.name.pl}>{c.name.pl}</option>
                ))}
              </select>
              <div className="flex gap-2">
                <select 
                  value={newSkill.minLevel}
                  onChange={e => setNewSkill({...newSkill, minLevel: parseInt(e.target.value)})}
                ...
                <select 
                  ...
                  onChange={e => setNewSkill({...newSkill, criticality: e.target.value as any})}
                  className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold"
                >
                ...
                <div key={skill.id} className="group flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-xl hover:border-indigo-100 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg border ${
                      skill.criticality === 'critical' ? 'bg-rose-50 text-rose-600 border-rose-200' :
                      skill.criticality === 'moderate' ? 'bg-amber-50 text-amber-600 border-amber-200' :
              ...
              <div key={employee.id} className="p-4 border border-slate-100 rounded-xl hover:border-indigo-200 transition-all shadow-sm bg-white flex flex-col">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className={`px-2 py-1 rounded-lg font-black text-[10px] ${
                    employee.match === 100 ? 'bg-emerald-500 text-white' :
                    employee.match > 50 ? 'bg-amber-500 text-white' :
