  ...
  static async chat(userId: string, tenantId: string, conversationId: string, message: string, history: any[] = []) {
    ...
    try {
      const response = await fetch('/api/v1/ai/chat', {
        ...
        body: JSON.stringify({ prompt: message, history, context: systemInstruction })
      });

      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Błąd zapytania do AI");
      }

      const responseText = data.text || "Przepraszam, wystąpił problem z przetworzeniem odpowiedzi.";
