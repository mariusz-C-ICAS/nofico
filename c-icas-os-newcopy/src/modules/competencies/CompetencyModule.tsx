...
export default function CompetencyModule() {
  ...
  const saveConfig = async (newConfig: typeof config) => {
    ...
    const docData = {
      managementLevel: newConfig.detailLevel,
      isAiOptimized: newConfig.autoAiSync,
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl shadow-inner divide-x divide-slate-200">
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                activeTab === 'dictionary' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                activeTab === 'architect' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                activeTab === 'settings' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
        ...
        {activeTab === 'settings' && (
          <div className="max-w-2xl mx-auto space-y-6">
              ...
              <div className="space-y-10">
                    ...
                    {(['SIMPLE', 'ADVANCED', 'EXPERT'] as const).map(level => (
                      <button 
                        ...
                        className={`p-5 rounded-2xl border-2 transition-all text-left ${
                          config.detailLevel === level ? 'border-indigo-600 bg-indigo-50 shadow-lg shadow-indigo-50' : 'border-slate-100 hover:border-slate-200 bg-white'
                        }`}
                      >
                ...
                <div className="flex items-center justify-between p-6 bg-slate-50 border border-slate-100 rounded-2xl">
                   <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium mt-0.5">Automatycznie analizuj kompetencje przy dodawaniu sprzętu/pracowników.</p>
                   </div>
                   <button 
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm space-y-6">
               ...
               <p className="text-xs text-slate-500 font-medium leading-relaxed">Pobierz predefiniowane katalogi kompetencji dla konkretnych sektorów (tysiące wzorców opracowanych przez ekspertów).</p>
